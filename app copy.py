...
if not df_entradas_mes.empty:
    st.download_button(
        label="Baixar documentos das compras do mês",
        data=entradas.dataframe_para_csv_br(df_entradas_mes),
        file_name="entradas_compras_mes.csv",
        mime="text/csv"
    )
        ...
        linhas.append(
            {
                ...
                "Variação %": f"{area.delta_perc:.2f}%".replace(".", ","),
            }
        )
